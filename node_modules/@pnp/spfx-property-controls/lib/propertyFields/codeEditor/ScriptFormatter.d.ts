...
export declare class ScriptFormatter {
    ...
    private create_flags(flags_base, mode);
    private create_output_line();
    private trim_output(eat_newlines);
    private trim_output_line(line);
    private trim(s);
    private split_newlines(s);
    private just_added_newline();
    private just_added_blankline();
    private allow_wrap_or_preserved_newline(force_linewrap);
    private print_newline(force_newline, preserve_statement_flags);
    private print_token_line_indentation();
    private print_indent_string(level);
    private print_token_space_before();
    private print_token(printable_token);
    private indent();
    private deindent();
    private remove_redundant_indentation(frame);
    private set_mode(mode);
    private is_array(mode);
    private is_expression(mode);
    private restore_mode();
    private start_of_object_property();
    private start_of_statement();
    private all_lines_start_with(lines, c);
    private is_special_word(word);
    private in_array(what, arr);
    private unescape_string(s);
    private is_next(find);
